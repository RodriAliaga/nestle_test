...
def main():
    server_url = "opc.tcp://10.115.53.54:48010"
    ...
    try:
        ...
        try:
            with resources.path("Resources", "nestle_plc_string.json") as path:
                with open(path) as f:
                    df = json.load(f)
        ...
        for el in df:
            try:
                ns = el["ns"]
                string_id = el["s"]
                tag_name = el["Descripcion"]  # Changed from "Description" to "Descripcion"
